...
Usuario* Biblioteca::buscaUsuario(string login, string clave) {
    ...
    for (unsigned i = 0; i < usur.tamanio(); i++) {
        if (usur.lee(i)->daLogin() == login && usur.lee(i)->daClave() == clave) {
            return usur.lee(i);
        }
    }
...
void Biblioteca::cargaLibros(string fichero) {
    ...
    if (entrada) {
        while (!entrada.eof()) {
            ...
            Libro *lib = new Libro(aTitulo, aAutores, aEditorial, aISBN, atoi(aAnio.c_str()), (float) atof(aPrecioActual.c_str()));
            libro.aumenta(lib);
        }
...
lista_sin<Libro *> * Biblioteca::consultaLibros(string titulo) {
    ...
    if (libros->tamanio() == 0) throw excepcionesBi::libroNoencontrado();
    else return libros;
...
lista_sin<PedidoUsuario *> * Biblioteca::buscaPedidosUsuarioPendientes(Usuario *usuario) {
    ...
    while (i < pedido_usu.tamanio()) {
        if (pedido_usu.lee(i)->daTramitado() == false && usu->daLogin(pedido_usu.lee(0)->daUsuario()) == usuario->daLogin()) {
            pedPendientes->aumenta(pedido_usu.lee(i));
        }
        i++;
    }
    if (pedPendientes->tamanio() != 0) return pedPendientes;
    else throw excepcionesBi::pedidoUsuarioNoencontrado();
...
lista_sin<PedidoUsuario *> * Biblioteca::buscaPedidosUsuarioTramitados(Usuario *usuario) {
    ...
    while (i < pedido_usu.tamanio()) {
        if (pedido_usu.lee(i)->daTramitado() == true && usu->daLogin(pedido_usu.lee(0)->daUsuario()) == usuario->daLogin()) {
...
lista_sin<PedidoBiblioteca *> * Biblioteca::buscaPedidosBibliotecaPendientes() {
    ...
    unsigned i = 0;
    lista_sin<PedidoBiblioteca *> * biPendientes = new lista_sin<PedidoBiblioteca *>;
    if (biPendientes->tamanio() == 0) throw excepcionesBi::pedidoBibliotecaNoencontrado();
    ...
    if (biPendientes->tamanio() != 0) return biPendientes;
    else throw excepcionesBi::pedidoBibliotecaNoencontrado();
}
...
lista_sin<PedidoBiblioteca *> * Biblioteca::buscaPedidosBibliotecaTramitados() {
    ...
    if (biTramitados->tamanio() != 0) return biTramitados;
    else throw excepcionesBi::pedidoBibliotecaNoencontrado();
}
